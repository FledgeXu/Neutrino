...
这两个方法是在区块刚被载入时会被调用的方法，之所以有这两个方法存在，是因为有些方块实体并不需要经常性的同步数据，比如告示牌。这种装饰性的方块实体，只需要在区块被载入时同步一次就行。
...
在这次我们只是简单的调用了之前的两个方法而已，`SUpdateTileEntityPacket`的第二个参数的序列号值，大家可以随便填写。
...
为了大家理解的方面我这里用了两个`if`语句将客户端和服务端的逻辑区分了开来
...
`playSound`的第一二个变量很好懂，第三个变量是需要播放的声音，第三个变量是控制这个声音大小是受到哪一个声音控制分类控制的。
